...
float noise(vec2 x){
    vec2 p = floor(x), f = fract(x); float n = p.x + p.y*32.0, res = 1.0;

    res = mix(mix(mat(n-1), mat(n),f.x), mix(mat(n+31), mat(n+32),f.x),f.y);
...
void main() {
	...
	vec4 worldposition = vec4(0.0), worldpositionraw = vec4(0.0);
	
	worldposition = gbufferModelViewInverse * fragposition;	
	
        float horizon = abs(worldposition.y - texcoord.y);

	float skyPos = max(pow(max(1.0 - horizon/3500.0, 0.01), 8.0)-0.1, 0.0);
	      skyPos = clamp(skyPos, 0.25, 1.0);
	...
	color.rgb *= skyGradient * skyPos;
	color.rgb = mix(color.rgb, skyGradient * 0.6, vista);
	...
	if (land < 0.1) {
	color.rgb += (basic_colour * getCNoise(fragposition, 32, 2.0, 2));
