...
const float mulfov = 1.0;
	...
	worldposition.xy *=  1.0/distortFactor/0.97; 
	if (max(abs(worldposition.x),abs(worldposition.y)) < 1.0) {

			float diffthresh = sunlightMat.a > 0.9? 0.0004 : distortFactor*distortFactor*(0.008*tan(acos(max(diffuse,0.0))) + 0.001)*0.2;
...
else albedo = vec4(0.0,0.0,0.,0.0);
