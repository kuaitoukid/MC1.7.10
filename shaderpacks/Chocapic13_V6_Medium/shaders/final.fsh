...
void main() {
	...
	#ifdef MOTIONBLUR
	if (texture2D(gnormal,texcoord).x>0.00015 || texture2D(gnormal,texcoord).x<0.00005){
		vec4 currentPosition = vec4(texcoord,texture2D(depthtex0,texcoord).x, 1.0)*2.0-1.0;
		...
		for (int i = 0; i < 15; ++i, coord += velocity) {
			...
			if (texture2D(gnormal,coord).x>0.00015 || texture2D(gnormal,coord).x<0.00005){
			color += texture2D(gcolor, coord).xyz;
			++samples;
			}
